...
.cg-nav-left { display: flex; align-items: center; gap: 18px; }
.cg-brand { display: flex; align-items: center; gap: 9px; }
.cg-brand-icon {
...
.cg-brand-name {
  ...
  letter-spacing: -.025em; color: var(--t0);
}
.cg-nav-tag {
  font-size: 10px; font-weight: 600;
  letter-spacing: .05em; text-transform: uppercase;
  color: var(--acc); background: var(--acc-g);
  border: 1px solid var(--acc-b); border-radius: 4px;
  padding: 2px 8px;
}
...
.cg-online {
  ...
  background: var(--ok); box-shadow: 0 0 6px rgba(34,197,94,0.5);
}
...
.cg-h1 {
  ...
  line-height: 1.1; color: var(--t0); margin: 0 0 16px;
}
.cg-tagline {
  font-size: 14px; line-height: 1.65;
  color: var(--t2); max-width: 520px; margin: 0 0 32px;
}

.cg-scan-box {
  background: var(--s1); border: 1px solid var(--b1);
  border-radius: var(--rxl); padding: 22px; margin-bottom: 32px;
}
.cg-field-label {
  font-size: 11px; font-weight: 500;
  letter-spacing: .05em; text-transform: uppercase;
...
.cg-stat-val {
  ...
  letter-spacing: -.04em; line-height: 1; color: var(--t0);
}
.cg-stat-val.ok { color: #4ade80; }
.cg-stat-val.warn { color: #fbbf24; }
.cg-stat-val.err { color: #f87171; }
.cg-stat-sub { font-size: 12px; color: var(--t2); margin-top: 6px; }

.cg-card {
  background: var(--s1); border: 1px solid var(--b1);
  border-radius: var(--rl); padding: 22px; height: 100%;
}
.cg-card-lbl {
  font-size: 10px; font-weight: 600;
  letter-spacing: .07em; text-transform: uppercase;
...
.cg-kv {
  ...
  border-bottom: 1px solid var(--b0); align-items: baseline;
}
.cg-kv:last-child { border-bottom: none; padding-bottom: 2px; }
.cg-kv-k {
  font-size: 11px; font-weight: 500;
  color: var(--t2); min-width: 82px; flex-shrink: 0;
}
...
.cg-sh {
  ...
  margin: 32px 0 12px; padding-bottom: 12px;
  border-bottom: 1px solid var(--b1);
}
.cg-sh-title {
  font-size: 14px; font-weight: 600;
  color: var(--t0); letter-spacing: -.015em;
}
...
.cg-fi-name {
  ...
  color: var(--t0); flex: 1; line-height: 1.3;
}
.cg-fi-desc {
  font-size: 12px; color: var(--t2);
  margin: 0 0 6px 14px; line-height: 1.5;
}
.cg-fi-foot {
  display: flex; align-items: center;
  gap: 8px; margin-left: 14px; flex-wrap: wrap;
}
.cg-chip {
  font-family: 'Fira Code', 'Cascadia Mono', monospace;
  font-size: 11px; color: var(--t2); background: var(--s3);
  border: 1px solid var(--b0); padding: 2px 8px; border-radius: 5px;
...
.cg-export-info { font-size: 13px; color: var(--t2); line-height: 1.7; }
.cg-export-info ul { margin: 8px 0 0; padding-left: 16px; }
.cg-export-info li { margin-bottom: 3px; }
...
_HDRS = {
    ...
    "X-Content-Type-Options": {"w": 8, "sev": "Medium",
        "desc": "Prevents browsers from MIME-sniffing, blocking content-type confusion attacks.",
        "fix": "X-Content-Type-Options: nosniff"},
    "Referrer-Policy": {"w": 6, "sev": "Low",
        "desc": "Controls how much referrer information is passed to other origins.",
        "fix": "Referrer-Policy: strict-origin-when-cross-origin"},
    "Permissions-Policy": {"w": 6, "sev": "Low",
        "desc": "Restricts which browser APIs and device features this page may use.",
...
def _proto(iu, fu):
    return [
        {"cat": "Protocol", "name": "HTTPS on input URL", "ok": iu.startswith("https://"),
         ...
         "desc": "Verifies the target URL uses HTTPS from the start.",
         "fix": "Ensure all traffic begins on https://."},
        {"cat": "Protocol", "name": "HTTPS on final URL", "ok": fu.startswith("https://"),
         "sev": "High", "w": 14, "val": fu,
         "desc": "Verifies the final page after redirects uses HTTPS.",
         "fix": "Issue a 301 redirect from HTTP to HTTPS."},
    ]
...
def _check_headers(resp):
    ...
    for h, m in _HDRS.items():
        ...
        out.append({"cat": "Security Headers", "name": h, "ok": p,
            ...
            "desc": m["desc"], "fix": "Configured correctly." if p else m["fix"]})
    for h in _OPT:
        p = h in resp.headers
        out.append({"cat": "Security Headers", "name": h, "ok": p,
            "sev": "Info" if p else "Low", "w": 3,
            "val": resp.headers.get(h, "Not set"),
            "desc": "Cross-origin isolation header for defence-in-depth hardening.",
            "fix": "Configured." if p else f"Consider adding {h}."})
    return out
...
def _check_cookies(resp):
    ...
    for i, c in enumerate(raw, 1):
        ...
        out.append({"cat": "Cookies", "name": f"Cookie #{i}", "ok": not miss,
            ...
            "desc": "Cookie security attributes protect against interception and forgery.",
...
def _check_disclosure(resp):
    ...
    for h, lbl in [("Server", "Server header"), ("X-Powered-By", "X-Powered-By header")]:
        ...
        out.append({"cat": "Information Disclosure", "name": lbl, "ok": not bool(val),
            ...
            "desc": "Exposing server/framework versions helps attackers find vulnerabilities.",
            "fix": "Not exposed." if not val else f"Remove the {h} response header."})
    return out

def _check_paths(base):
    ...
    for p in _PATHS:
        ...
        try:
            ...
            out.append({"cat": "Security Files", "name": p, "ok": ok,
                ...
                "desc": "Standard file that aids security researchers and disclosure.",
...
def _make_summary(findings, ssl_info):
    ...
    if not pts:
        pts.append("No critical security configuration issues were identified.")
    return pts
...
def run_scan(url):
    ...
    findings = _proto(t, resp.url) + _check_headers(resp) + _check_cookies(resp) + _check_disclosure(resp) + _check_paths(resp.url)
    score, rating = _calc_score(findings, ssl_info)
    return {
        "input_url": t, "final_url": resp.url, "hostname": h,
...
def _ps(n, sz, col, bold=False, lead=None, sb=0, sa=4, indent=0):
    return ParagraphStyle(n, fontSize=sz, textColor=col,
        ...
        leading=lead or round(sz * 1.5),
        spaceBefore=sb, spaceAfter=sa, leftIndent=indent)

def _sev_c(s):
    return {"High": PC["red"], "Medium": PC["amber"], "Low": PC["blue"], "Info": PC["green"]}.get(s, PC["muted"])

def _rat_c(r):
    return {"Strong": PC["green"], "Moderate": PC["amber"],
            "Needs Improvement": PC["amber"], "High Risk": PC["red"]}.get(r, PC["muted"])

def _hl(sa=8):
    return HRFlowable(width="100%", thickness=0.5, color=PC["bdr"], spaceAfter=sa)
...
class _HF:
    ...
    def __call__(self, cv, doc):
        W, H = A4
        cv.saveState()
        cv.setFillColor(PC["navy"])
        cv.rect(0, H - 50, W, 50, fill=1, stroke=0)
        cv.setFillColor(PC["blue"])
        cv.rect(0, H - 50, 4, 50, fill=1, stroke=0)
        cv.setFont("Helvetica-Bold", 13)
        cv.setFillColor(PC["white"])
        ...
        cv.setFillColor(PC["bg2"])
        cv.rect(0, 0, W, 26, fill=1, stroke=0)
        cv.setStrokeColor(PC["bdr"])
        cv.setLineWidth(0.5)
        cv.line(0, 26, W, 26)
        cv.setFont("Helvetica", 7)
        cv.setFillColor(PC["sub"])
...
def build_pdf(scan, company, analyst):
    ...
    def kv_table(rows, k_w=38*mm):
        ...
        t.setStyle(TableStyle([
            ...
            ("BACKGROUND",    (0,0), (0,-1),  PC["bg2"]),
            ("BACKGROUND",    (1,0), (1,-1),  PC["bg"]),
            ("TEXTCOLOR",     (0,0), (0,-1),  PC["muted"]),
            ("TEXTCOLOR",     (1,0), (1,-1),  PC["text"]),
            ("FONTNAME",      (0,0), (0,-1),  "Helvetica-Bold"),
            ("LINEBELOW",     (0,0), (-1,-1), 0.5, PC["bdr"]),
        ]))
        return t
    ...
    title_row = Table([[Paragraph("Website Security<br/>Review Report", S["h1"]), score_tbl]],
        colWidths=[CW - 58*mm, 58*mm])
    title_row.setStyle(TableStyle([
        ("VALIGN",         (0,0), (-1,-1), "BOTTOM"),
        ("PADDING",        (0,0), (-1,-1), 0),
        ("LINEBELOW",      (0,0), (-1, 0), 0.8, PC["navy"]),
    ...
    ssl = scan["ssl_info"]
    d_remaining = ssl.get("days_remaining", 0) or 0
    dc = PC["green"] if d_remaining > 45 else PC["amber"] if d_remaining > 15 else PC["red"]
    ssl_val = "Active" if ssl.get("enabled") else "None"
    ssl_col = PC["green"] if ssl.get("enabled") else PC["red"]
    issues_count = sum(1 for f in scan["findings"] if not f["ok"])
    iss_col = PC["amber"] if issues_count > 0 else PC["green"]

    def ov_cell(label, val, col, sub=""):
        rows = [
            [Paragraph(f'<font size="7" color="#64748b"><b>{label}</b></font>', S["mu"])],
            [Paragraph(f'<font size="17" color="{col.hexval()}"><b>{val}</b></font>', S["body"])],
        ]
        if sub:
            rows.append([Paragraph(f'<font size="7" color="#94a3b8">{sub}</font>', S["mu"])])
    ...
    ov = Table([[
        ov_cell("SCORE",   f'{scan["score"]}/100', rc, scan["rating"]),
        ov_cell("TLS",     ssl_val, ssl_col, f'{d_remaining} days' if ssl.get("enabled") else ""),
        ov_cell("HTTP",    str(scan["status_code"]), PC["blue"], "Response code"),
        ov_cell("ISSUES",  str(issues_count), iss_col, f'of {len(scan["findings"])} checks'),
    ]], colWidths=[(CW - 3*mm) / 4] * 4)
    ov.setStyle(TableStyle([
        ("BACKGROUND", (0,0), (-1,-1), PC["bg1"]),
        ("BOX",        (0,0), (-1,-1), 0.5, PC["bdr"]),
        ("LINEBEFORE", (1,0), (3, 0),  0.5, PC["bdr"]),
        ("PADDING",    (0,0), (-1,-1), 0),
        ("VALIGN",     (0,0), (-1,-1), "MIDDLE"),
    ]))
    story += [ov, Spacer(1, 4)]
    ...
    if ssl.get("enabled"):
        story.append(kv_table([
            ...
            ("Days Left", f'<font color="{dc.hexval()}"><b>{d_remaining}</b></font>'),
        ]))
    else:
    ...
    for pt in scan["summary"]:
        story.append(Paragraph(
            f"- {pt}",
            _ps("bp", 9, PC["text"], lead=14, sa=3, indent=8)
        ))
    story.append(Spacer(1, 4))
    ...
    hdr_row = [Paragraph(f"<b>{t}</b>", S["h3"])
               for t in ["Finding", "Category", "Severity", "Result", "Recommendation"]]
    rows = [hdr_row]
    for f in scan["findings"]:
        sc2 = _sev_c(f["sev"])
        pc = PC["green"] if f["ok"] else PC["red"]
        rows.append([
    ...
    ft.setStyle(TableStyle([
        ...
        ("VALIGN",         (0,0), (-1,-1), "TOP"),
        ("BACKGROUND",     (0,0), (-1, 0), PC["navy"]),
        ("TEXTCOLOR",      (0,0), (-1, 0), PC["white"]),
        ("FONTNAME",       (0,0), (-1, 0), "Helvetica-Bold"),
        ("ROWBACKGROUNDS", (0,1), (-1,-1), [PC["bg"], PC["bg1"]]),
        ("LINEBELOW",      (0,0), (-1,-1), 0.4, PC["bdr"]),
        ("BOX",            (0,0), (-1,-1), 0.5, PC["bdr"]),
    ]))
    story.append(ft)
    ...
    disc_data = [[Paragraph(
        ...
        _ps("di", 8, PC["muted"], lead=12))]]
    disc = Table(disc_data, colWidths=[CW])
    disc.setStyle(TableStyle([
        ("BACKGROUND",    (0,0), (-1,-1), PC["bg2"]),
        ("LINEABOVE",     (0,0), (-1, 0), 2, PC["blue"]),
    ...
    with mid:
        ...
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">
          <div style="width:32px;height:32px;border-radius:8px;
               ...
               display:flex;align-items:center;justify-content:center;
               font-size:16px;color:white;flex-shrink:0">S</div>
          <span style="font-size:16px;font-weight:700;letter-spacing:-.025em;color:#f4f4f6">CyberGuard</span>
        </div>
        <p style="font-size:12px;color:#60607a;margin:0 0 28px 0">
...
with st.sidebar:
    st.markdown(f"<p style='font-size:14px;font-weight:600;color:#f4f4f6;margin-bottom:3px'>{user['name']}</p>",
                unsafe_allow_html=True)
    st.markdown(f"<p style='font-size:11px;color:#60607a;margin-top:0'>{user['email']}</p>",
                unsafe_allow_html=True)
    st.divider()
...
<div class="cg-hero">
  <div class="cg-eyebrow">
    <span class="cg-eyebrow-dot"></span>
    Security Review Platform
  </div>
  <h1 class="cg-h1">Website security,<br>reviewed in seconds.</h1>
  <p class="cg-tagline">
    Passive defensive analysis across TLS, security headers, cookies, and information
    disclosure. Export a polished, client-ready PDF report in one click.
  </p>
</div>
""", unsafe_allow_html=True)
...
if scan_btn:
    if not target_url.strip():
        ...
        try:
            ...
            st.error("Could not connect. Check the URL and try again.")
...
if scan:
    ...
    <div class="cg-sh">
      ...
      <span class="cg-sh-meta">{issues} issue{'s' if issues != 1 else ''} across {total} checks</span>
    </div>""", unsafe_allow_html=True)
    ...
    with ex1:
        ...
        <div class="cg-export">
          ...
          <p class="cg-export-info">
            A structured A4 PDF including:
            <ul>
              <li>Engagement details and assessment overview</li>
              <li>TLS certificate summary with expiry status</li>
              <li>Executive summary with key findings</li>
              <li>Full findings table with severity, pass/fail, and remediation</li>
    ...
    with ex2:
        ...
        st.caption("Suitable for client, portfolio, and internal use.")

st.markdown('</div>', unsafe_allow_html=True)

st.markdown("""
<div class="cg-footer">
  <span>CyberGuard &nbsp;&middot;&nbsp; Pamupro Cyber</span>
  <span>Authorised defensive reviews only</span>
</div>
""", unsafe_allow_html=True)
